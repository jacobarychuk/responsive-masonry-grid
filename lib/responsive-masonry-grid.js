...
function Masonry({
  ...
  const [columnData, setColumnData] = useState(Array.from({
    length: columns
  }, () => []));
  ...
  useEffect(() => {
    ...
    const newColumnData = Array.from({
      length: columns
    }, () => []);
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      const newColumns = Math.min(maxColumns, Math.max(1, Math.floor((gridWidth + remSpacing) / (remColumnWidth + remSpacing))) // Account for gap between columns
      );
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    key: index,
    ref: itemRefs[index]
  }, item))), columnData && columnData.map((columnItems, columnIndex) => /*#__PURE__*/React.createElement("div", {
    key: columnIndex
  }, columnItems.map((item, itemIndex) => /*#__PURE__*/React.createElement("div", {
    key: itemIndex,
    style: {
      marginBottom: itemIndex !== columnItems.length - 1 ? remSpacing : 0
    }
  }, item)))));
}
